...
void re_locate(int seg,int desc,int is,int nsam)
{
    int j,count,tdesc;
    int stack[nsam*2];
    count=1;
    ...
    while(count!=0)
    {
        ...
        if(tdesc<nsam)
        {
            ++breaks[tdesc][0].pos;
            struct bpoint *tmp;
            tmp=breaks[tdesc][0].next;
            breaks[tdesc][0].next=(struct bpoint*)malloc(sizeof(struct bpoint));
            breaks[tdesc][0].next->pos=is;
            breaks[tdesc][0].next->next=tmp;
            continue;
        }
        for(j=0;j<=nnodes[seg];++j)
        {
            if((seglst[seg].ptree+j)->abv==tdesc)
            {
                stack[count]=j;
                ++count;
            }
...
void refind(int ic,int is,int nsam)
{
    int seg,i,j,desc;
    desc=-1;
    for(i=0;i<chrom[ic].nseg;++i)
    {
        if(is>=chrom[ic].pseg[i].beg && is<=chrom[ic].pseg[i].end)
        {
            desc=chrom[ic].pseg[i].desc;
        }
    }
    if(desc!=-1)
    {
        for(seg=0,i=0;i<nsegs;seg=seglst[seg].next,++i)
        {
            if(i==nsegs-1 || (seglst[seg].beg<=is && seglst[seglst[seg].next].beg>is))
            {
                re_locate(seg,desc,is,nsam);
                return;
            }
        }
    }
...
/* Initialization */
    ...
    long li,lj;
    breaks=(struct bpoint**)malloc(sizeof(struct bpoint*)*nsam);
    for(i=0;i<nsam;++i)
    {
        breaks[i]=(struct bpoint*)malloc(sizeof(struct bpoint));
        breaks[i][0].pos=0;
        breaks[i][0].next=NULL;
    }
    int cnsam=nsam;
...
//			   printf("Coalescent Event\n");
 ...
 /*       int tmpi,tmpj;
        for(tmpi=0;tmpi<nchrom;++tmpi)
        {
            for(tmpj=0;tmpj<chrom[tmpi].nseg;++tmpj)
                printf("%d,%d,%d\t",chrom[tmpi].pseg[tmpj].beg,chrom[tmpi].pseg[tmpj].end,chrom[tmpi].pseg[tmpj].desc);
            printf("\n");
        }
        for(i=0,j=0;j<nsegs;i=seglst[i].next,++j)
        {
            printf("beg=%d,next=%d\n",seglst[i].beg,seglst[i].next);
            printf("nnode=%d\n",nnodes[i]);
            for(k=0;k<=nnodes[i];++k)
                printf("%d:%d\t",k,(seglst[i].ptree+k)->abv);
            printf("\n");
        }
	printf("*\n");
  */
	     }
    struct bpoint* t_bp,*tt_bp;
    for(i=0;i<nsam;++i)
    {
        printf("%d:",i);
        t_bp=breaks[i][0].next;
        for(li=0;li<breaks[i][0].pos;++li)
        {
            printf("\t%ld",t_bp->pos);
            t_bp=t_bp->next;
        }
